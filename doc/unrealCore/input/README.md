...
| 문서 | 내용 |
|------|------|
| [00. 입력이 도달하는 원리 (개요)](engine/00_how_input_arrives.md) | 5계층 전체 흐름 요약, 각 상세 문서 링크 |
| [00a. OS → PlayerInput 수신 경로](engine/00_os_to_playerinput.md) | OS(WM_KEYDOWN) → FWindowsApplication → UGameViewportClient → PlayerInput 수신 경로, 패드 폴링 vs 키보드 이벤트 |
| [00b. FSlateApplication 라우팅](engine/00_slate_routing.md) | ProcessKeyDownEvent 내부, FWidgetPath, Tunnel/Bubble 두 단계, SViewport→FSceneViewport 브릿지, 가로채기 3메커니즘 |
